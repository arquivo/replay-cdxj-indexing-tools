...
def process_pipeline(
    ...
    if verbose:
        ...
        print(f"Complete: Kept {lines_kept} lines, filtered {lines_filtered} lines ({pct:.1f}%)", 
              file=sys.stderr)
...
Examples:
  ...
  %(prog)s auto -i arquivo.cdxj -o cleaned.cdxj -n 500 -v
    ...
    auto_parser.add_argument(
        '-i', '--input',
        required=True,
        help='Input CDXJ file (cannot be stdin for auto mode)'
    )
    auto_parser.add_argument(
        '-o', '--output',
        default='-',
    ...
    try:
        if args.command == 'find':
            ...
            print(f"# Found {len(excessive)} URLs with > {args.threshold} occurrences",
                  file=sys.stderr)
            ...
            total = lines_kept + lines_filtered
            pct = (lines_filtered / total * 100) if total > 0 else 0
            print(f"# Kept {lines_kept} lines, filtered {lines_filtered} lines ({pct:.1f}%)",
                  file=sys.stderr)
