...
class TestFindExcessiveUrls(unittest.TestCase):
    ...
    def test_realistic_arquivo_scenario(self):
        ...
        lines.extend(['pt,governo,www)/noticias 20230101%06d {"url": "..."}\n' % i for i in range(5000)])
        # Sapo mail with same path (typical crawler trap - same URL over and over)
        lines.extend(['pt,sapo,mail)/inbox 20230101%06d {"url": "..."}\n' % i for i in range(3000)])
        
        # Medium sites
        lines.extend(['pt,publico,www)/economia/artigo%d 20230101%06d {"url": "..."}\n' % (i, i) for i in range(500)])
        lines.extend(['pt,rtp,www)/noticias/%d 20230101%06d {"url": "..."}\n' % (i, i) for i in range(800)])
        
        # Long tail (many small sites)
        for domain_id in range(50):
            lines.extend([f'pt,domain{domain_id},www)/ 202301010000{i:02d} {{"url": "..."}}\n' for i in range(10)])
...
class TestProcessPipeline(unittest.TestCase):
    ...
    def test_auto_mode_basic(self):
        ...
        excessive_count, kept, filtered = process_pipeline(
            input_path, output_path, threshold=10
        )
    ...
    def test_auto_mode_no_excessive(self):
        ...
        excessive_count, kept, filtered = process_pipeline(
            input_path, output_path, threshold=100
        )
    ...
    def test_auto_mode_arquivo_pipeline(self):
        ...
        lines.extend([f'pt,governo,www)/page{i} 20230101{i:06d} {{"url": "..."}}\n' for i in range(200)])
        lines.extend([f'pt,sapo,www)/artigo{i} 20230101{i:06d} {{"url": "..."}}\n' for i in range(300)])
        
        # Crawler trap (same URL repeated many times)
        lines.extend(['pt,trap,www)/loop 20230101120000 {"url": "..."}\n'] * 2000)
        
        # More legitimate content
        lines.extend([f'pt,publico,www)/news{i} 20230101{i:06d} {{"url": "..."}}\n' for i in range(150)])
        ...
        excessive_count, kept, filtered = process_pipeline(
            input_path, output_path, threshold=1000
        )
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_large_file_performance(self):
        ...
        self.assertLess(elapsed, 5.0, 
                       f"Processing 100k lines took {elapsed:.1f}s, should be < 5s")
